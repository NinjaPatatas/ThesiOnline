...
if(isset($_POST['generate'])){
    ...
    $sqldelete="DELETE FROM projection";
    $resdelete=mysqli_query($con,$sqldelete);
    
    $sqldates="SELECT DISTINCT dateregister FROM student where dateregister>='".$_POST['starting']."' and dateregister<='".$_POST['Ending']."'";

    $resdates=mysqli_query($con,$sqldates);
    while($rowdates=mysqli_fetch_array($resdates)){
        $sqlstudent="SELECT * from student where dateregister='".$rowdates['dateregister']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['dateregister'];
        
        $adddate="INSERT into projection (enrolled_date,registered_count)values('".$dateregister."',".$countstudent.")";
        $resdate=mysqli_query($con,$adddate);
    }
    
    $sqldates1="SELECT DISTINCT dateenrolled FROM student where dateenrolled>='".$_POST['starting']."' and dateenrolled<='".$_POST['Ending']."'";
    $resdates=mysqli_query($con,$sqldates1);
    while($rowdates=mysqli_fetch_array($resdates)){
        $checksql="SELECT * from projection where enrolled_date='".$rowdates['dateenrolled'] ."'";
        $checkres=mysqli_query($con,$checksql);
        if($checkcount=mysqli_num_rows($checkres)){
        $sqlstudent="SELECT * from student where dateenrolled='".$rowdates['dateenrolled']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['dateenrolled'];
        
        $adddate="UPDATE projection set enrolled_count=".$countstudent." where enrolled_date='".$rowdates['dateenrolled']."'";
        $resdate=mysqli_query($con,$adddate); 
        ...
        $adddate="INSERT into projection (enrolled_date,enrolled_count)values('".$dateregister."',".$countstudent.")";
        $resdate=mysqli_query($con,$adddate);
...
<div class="content-wrapper">
    ...
    <section class="content" >
      <div class="container-fluid">
        ...
        <div class="col-sm-6">
            <a href="projection.php" id="printButton1" class="btn btn-glass scrollto" style="background-color: white; color: #12543b;">Daily</a>
        ...
        <section class="content" id="mySection">
            ...
            <div class="col-12">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <?php  
                        $query1="SELECT * from projection ORDER BY enrolled_date DESC";
                        $res=mysqli_query($con,$query1);
                        while($row=mysqli_fetch_array($res)):
                      ?>

                      <tr>
                        <td  style=" text-align: center;"><?php echo $row['enrolled_date'];?></td>
...
<script>
  $(function () {
    ...
    var areaChartData = {
      labels  : [<?php 
      $sqlpro="SELECT * from projection order by enrolled_date";
      $respro=mysqli_query($con,$sqlpro);
      while($rowpro=mysqli_fetch_array($respro)){
          echo "'".$rowpro['enrolled_date']."',";
      }
