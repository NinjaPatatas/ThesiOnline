...
if(isset($_POST['generate'])){
    ...
    $sqldelete="DELETE FROM projectionmonth";
    $resdelete=mysqli_query($con,$sqldelete);
    
    $sqldates="SELECT YEAR(dateregister) as year_number, MONTH(dateregister) as month_number, MIN(dateregister) as startmonth, MAX(dateregister) as endmonth FROM student where dateregister>='".$_POST['starting']."' and dateregister<='".$_POST['Ending']."' GROUP BY year_number,month_number order by year_number,month_number";
    $resdates=mysqli_query($con,$sqldates);
    
    while($rowdates=mysqli_fetch_array($resdates)){
        $sqlstudent="SELECT * from student where dateregister>='".$rowdates['startmonth']."' and dateregister<='".$rowdates['endmonth']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['year_number'];
        $monthregister=$rowdates['month_number'];
        $weekrange=$rowdates['startmonth']." to ".$rowdates['endmonth'];
        
        $adddate="INSERT into projectionmonth (ProjectionYear,ProjectionMonth,Registered_Range,Registered_Count)values('".$dateregister."','".$monthregister."','".$weekrange."', '".$countstudent."')";
        $resdate=mysqli_query($con,$adddate);
    }
    
    $sqldates="SELECT YEAR(dateenrolled) as year_number, MONTH(dateenrolled) as month_number, MIN(dateenrolled) as startmonth, MAX(dateenrolled) as endmonth FROM student where dateenrolled>='".$_POST['starting']."' and dateenrolled<='".$_POST['Ending']."' GROUP BY year_number,month_number order by year_number,month_number";
    $resdates=mysqli_query($con,$sqldates);
    
    while($rowdates=mysqli_fetch_array($resdates)){
        $checksql="SELECT * from projectionmonth where ProjectionYear='".$rowdates['year_number']."' and ProjectionMonth='".$rowdates['month_number']."'";
        $checkres=mysqli_query($con,$checksql);
        if($checkcount=mysqli_num_rows($checkres)){
        $sqlstudent="SELECT * from student where dateenrolled>='".$rowdates['startmonth']."' and dateenrolled<='".$rowdates['endmonth']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['year_number'];
        $monthregister=$rowdates['month_number'];
        ...
        $adddate="UPDATE projectionmonth set Enrolled_Count=".$countstudent.", Enrolled_Range='".$weekrange."' where ProjectionYear='".$rowdates['year_number']."' and ProjectionMonth='".$rowdates['month_number']."'";
        $resdate=mysqli_query($con,$adddate); 
        ...
        $adddate="INSERT into projectionmonth (ProjectionYear,ProjectionMonth,Enrolled_Range,Enrolled_Count)values('".$dateregister."','".$monthregister."','".$weekrange."', '".$countstudent."')";
        $resdate=mysqli_query($con,$adddate);
    ...
    <section class="content" >
      <div class="container-fluid">
        ...
        <div class="col-sm-6">
            ...
            <a href="proweek.php" id="printButton2" class="btn btn-glass scrollto" style="background-color: #12543b; color: #ffffff;">Weekly</a>
            <a href="promonth.php" id="printButton3" class="btn btn-glass scrollto" style="background-color: white; color: #12543b;">Monthly</a>
        ...
        <section class="content" id="mySection">
            ...
            <div class="col-12">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <?php  
                        $query1="SELECT * from projectionmonth ORDER BY ProjectionMonth DESC";
                        $res=mysqli_query($con,$query1);
                        while($row=mysqli_fetch_array($res)):
                      ?>

                      <tr>
                        <td  style=" text-align: center;"><?php echo $row['ProjectionYear']."-".$row['ProjectionMonth'];?></td>
                        <td  style=" text-align: center;"><?php echo $row['Registered_Range'];?></td>
                        <td style=" text-align: center;"><?php echo $row['Registered_Count'];?></td>
                        <td  style=" text-align: center;"><?php echo $row['Enrolled_Range'];?></td>
...
<script>
  $(function () {
    ...
    var areaChartData = {
      labels  : [<?php 
      $sqlpro="SELECT * from projectionmonth order by ProjectionYear,ProjectionMonth";
      $respro=mysqli_query($con,$sqlpro);
      while($rowpro=mysqli_fetch_array($respro)){
          echo "'".$rowpro['ProjectionMonth']."-".$rowpro['ProjectionYear']."',";
      }
