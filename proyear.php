...
if(isset($_POST['generate'])){
    ...
    $sqldelete="DELETE FROM projectionyear";
    $resdelete=mysqli_query($con,$sqldelete);
    
    $sqldates="SELECT YEAR(dateregister) as year_number, MIN(dateregister) as startyear, MAX(dateregister) as endyear FROM student where dateregister>='".$_POST['starting']."' and dateregister<='".$_POST['Ending']."' GROUP BY year_number order by year_number";
    $resdates=mysqli_query($con,$sqldates);
    
    while($rowdates=mysqli_fetch_array($resdates)){
        $sqlstudent="SELECT * from student where dateregister>='".$rowdates['startyear']."' and dateregister<='".$rowdates['endyear']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['year_number'];
        $weekrange=$rowdates['startyear']." to ".$rowdates['endyear'];
        
        $adddate="INSERT into projectionyear (ProjectionYear,Registered_Range,Registered_Count)values('".$dateregister."','".$weekrange."', '".$countstudent."')";
        $resdate=mysqli_query($con,$adddate);
    }
    
    $sqldates="SELECT YEAR(dateenrolled) as year_number, MIN(dateenrolled) as startyear, MAX(dateenrolled) as endyear FROM student where dateenrolled>='".$_POST['starting']."' and dateenrolled<='".$_POST['Ending']."' GROUP BY year_number order by year_number";
    $resdates=mysqli_query($con,$sqldates);
    
    while($rowdates=mysqli_fetch_array($resdates)){
        $checksql="SELECT * from projectionyear where ProjectionYear='".$rowdates['year_number']."'";
        $checkres=mysqli_query($con,$checksql);
        if($checkcount=mysqli_num_rows($checkres)){
        $sqlstudent="SELECT * from student where dateenrolled>='".$rowdates['startyear']."' and dateenrolled<='".$rowdates['endyear']."'";
        $resstudent=mysqli_query($con,$sqlstudent);
        $countstudent=mysqli_num_rows($resstudent);
        $dateregister=$rowdates['year_number'];
        $weekrange=$rowdates['startyear']." to ".$rowdates['endyear'];
        
        $adddate="UPDATE projectionyear set Enrolled_Count=".$countstudent.", Enrolled_Range='".$weekrange."' where ProjectionYear='".$rowdates['year_number']."'";
        $resdate=mysqli_query($con,$adddate); 
        ...
        $adddate="INSERT into projectionyear (ProjectionYear,Enrolled_Range,Enrolled_Count)values('".$dateregister."','".$weekrange."', '".$countstudent."')";
        $resdate=mysqli_query($con,$adddate);
    ...
    <section class="content" >
      <div class="container-fluid">
        ...
        <div class="col-sm-6">
            ...
            <a href="proyear.php" id="printButton4" class="btn btn-glass scrollto" style="background-color: white; color: #12543b;">Annually</a>
        ...
        <section class="content" id="mySection">
            ...
            <div class="col-12">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <?php  
                        $query1="SELECT * from projectionyear ORDER BY ProjectionYear DESC";
                        $res=mysqli_query($con,$query1);
                        while($row=mysqli_fetch_array($res)):
                      ?>

                      <tr>
                        <td  style=" text-align: center;"><?php echo $row['ProjectionYear'];?></td>
...
<script>
  $(function () {
    ...
    var areaChartData = {
      labels  : [<?php 
      $sqlpro="SELECT * from projectionyear order by ProjectionYear";
      $respro=mysqli_query($con,$sqlpro);
      while($rowpro=mysqli_fetch_array($respro)){
          echo "'".$rowpro['ProjectionYear']."',";
      }
